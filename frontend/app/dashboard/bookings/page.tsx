...
const BookingsPage = () => {
  ...
  // Fetch bookings whenever organization changes
  useEffect(() => {
    if (currentOrgId) {
  ...
  // Filter bookings by customer name or service title
        ...
        <div className="grid gap-4">
          {filteredBookings.map((booking) => {
            ...
            return (
              <Card
                key={booking.id}
                className={`group relative overflow-hidden border-none shadow-sm hover:shadow-xl transition-all duration-300 rounded-[2rem] bg-card p-1 ${
